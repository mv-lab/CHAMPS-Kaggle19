...
class BNLSTMCell(nn.Module):
    ...
    def __init__(self, input_size, out_size, hidden_size, max_length, use_bias=True,
                 jit=False):
        ...
        self.use_bias = use_bias
        self.weight_ih = nn.Parameter(
            torch.FloatTensor(out_size, 4 * hidden_size))
        self.weight_hh = nn.Parameter(
            torch.FloatTensor(hidden_size, 4 * hidden_size))
        if use_bias:
            self.bias = nn.Parameter(torch.FloatTensor(4 * hidden_size))
    ...
    def forward(self, input_, hx, time):
        ...
        bias_batch = (self.bias.unsqueeze(0)
                      .expand(batch_size, *self.bias.size()))
        #print(h_0.shape, self.weight_hh.shape)
        wh = torch.mm(h_0, self.weight_hh)
        #print(input_.shape, self.weight_ih.shape)
        wi = torch.mm(input_, self.weight_ih)
...
class LSTM(nn.Module):
    ...
    def __init__(self, cell_class, input_size, out_size, hidden_size, num_layers=1,
        ...
        for layer in range(num_layers):
            ...
            cell = cell_class(input_size=layer_input_size, out_size=out_size,
